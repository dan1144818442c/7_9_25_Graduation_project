...
def get_id_transcription_and_updating_elastic():
    logger = log.Logger.get_logger()
    persister = Persister(index_name=config.INDEX_NAME, mapping_for_elastic=config.INDEX_MAPPING,
                          db_name=config.DB_NAME, uri=config.URI, collection_name="new")
    ...
    for id in all_ids_mongo_files:
        ...
        if not (config.NAME_FIELD_TRANSCRIPTION_AUDIO in doc_to_update.keys()):
            doc_to_update = transcription_and_updating_doc(
                field_name_for_Transcription=config.NAME_FIELD_TRANSCRIPTION_AUDIO, mongo_fs=persister.mongo.fs,
                id=id, doc_to_update=doc_to_update)
...
if __name__ == '__main__':
    ...
    while True:
        try:
            ...
            logger.info("get_id_transcription_and_updating_elastic for all document that have their id in mongodb")
        except Exception as e:
            logger.error(f"get_id_transcription_and_updating_elastic for all document that have their id in mongodb -- {e}")
